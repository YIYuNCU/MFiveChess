...
class GomokuWindow(QMainWindow):
    ...
    def paintEvent(self, e):
        ...
        def draw_map():
            ...
            # 绘制横线
            for x in range(19):
                qp.drawLine(40 * (x + 1), 40, 40 * (x + 1), 760)
            # 绘制竖线
            for y in range(19):
                qp.drawLine(40, 40 * (y + 1), 760, 40 * (y + 1))
            # 绘制棋盘中的黑点
            qp.setBrush(QColor(0, 0, 0))
            key_points = [(5, 5), (15, 5), (10, 10), (5, 15), (15, 15)]
            for t in key_points:
                qp.drawEllipse(QPoint(40 * t[0], 40 * t[1]), 6,6)
    ...
    def mouseMoveEvent(self, e):
        ...
        if 29 <= mouse_x <= 779 and 29 <= mouse_y <= 779 and (mouse_x % 40 <= 19 or mouse_x % 40 >= 29) and (mouse_y % 40 <= 19 or mouse_y % 40 >= 29):
            game_x = int((mouse_x + 19) // 40) - 1
            game_y = int((mouse_y + 19) // 40) - 1
    ...
    def mousePressEvent(self, e):
        ...
        if e.button() == Qt.LeftButton:
            ...
            mouse_x = e.windowPos().x()
            mouse_y = e.windowPos().y()
            if (mouse_x % 40 <= 19 or mouse_x % 40 >= 29) and (mouse_y % 40 <= 19 or mouse_y % 40 >= 29):
                game_x = int((mouse_x + 19) // 40) - 1
                game_y = int((mouse_y + 19) // 40) - 1
            else:  # 鼠标点击的位置不正确
                return
            ...
            if res != 0:
                ...
                self.game_restart(res)
                return
            self.repaint(0, 0, 720, 720)  # 在游戏还没有结束的情况下，显示游戏内容，并继续下一轮循环
    ...
    def end_flash(self):
        ...
        if self.flash_cnt <= 5:
            ...
            self.repaint(0, 0, 720, 720)  # 重新绘制游戏界面

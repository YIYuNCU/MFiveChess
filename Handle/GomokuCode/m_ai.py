                ...
                if self.game.g_map[x][y] == 1:
                    ...
                    for x0 in range(x - 1, x + 2):
                        for y0 in range(y - 1, y + 2):
                            if 0 <= x0 <= 18 and 0 <= y0 <= 18 and self.game.g_map[x0][y0] != 0:
                                around_cnt += 1
                    player_score_num += score_by_num_around[around_cnt] - abs(x - 9) - abs(y - 9)
                    player_cnt += 1
                if self.game.g_map[x][y] == 2:
                    ...
                    for x0 in range(x - 1, x + 2):
                        for y0 in range(y - 1, y + 2):
                            if 0 <= x0 <= 18 and 0 <= y0 <= 18 and self.game.g_map[x0][y0] != 0:
                                around_cnt += 1
                    ai_score_num += score_by_num_around[around_cnt] - abs(x - 9) - abs(y - 9)
                    ai_cnt += 1
...
class AI1Step:
    ...
    max_node_num = 500000  # 最大允许的节点数量（避免内存占用过多）
        ...
        for x in range(19):
            for y in range(19):
                if self.method_tree[cur_node_dx].game.g_map[x][y] != 0:
                    for x0 in range(x - 1, x + 2):
                        for y0 in range(y - 1, y + 2):
                            if 0 <= x0 <= 18 and 0 <= y0 <= 18 and (x0, y0) not in ope_list:
